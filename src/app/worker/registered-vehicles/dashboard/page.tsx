...
const SERVICE_NAMES = ["New Vehicle Registration", "Vehicle Ownership Transfer"];
...
export default function WorkerRegisteredVehiclesDashboard() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      setLoading(true);
      try {
        const appsQuery = query(collection(db, "applications"), where("service", "in", SERVICE_NAMES));
    ...
    <AdminLayout workerMode>
      <div className="flex-1 space-y-8 p-4 md:p-8 pt-6">
        <div className="flex items-center justify-between space-y-2">
          <h1 className="text-3xl font-bold tracking-tight">Vehicle Registration Dashboard</h1>
        </div>
        <Card>
          <CardHeader>
            <CardTitle>Registration & Transfer Applications</CardTitle>
            <CardDescription>Review and manage all submitted applications for vehicle services.</CardDescription>
                    ...
                    <TableRow key={app.id}>
                      ...
                      <TableCell className="text-right">
                        <Button asChild variant="outline" size="sm">
                          <Link href={`/worker/applications/${app.id}?from=/worker/registered-vehicles/dashboard`}>
